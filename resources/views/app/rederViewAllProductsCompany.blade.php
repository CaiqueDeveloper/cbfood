...
<div class="item-announcement w-full min-h-[250px] flex flex-col-reverse relative shadow-lg cursor-pointer my-[5px] overflow-hidden sm:rounded-3xl">
    ...
    <a href="#" class="getModalProduct" value="{{$product->id}}">
        <div class="item-announcement-body absolute w-full h-full top-0 left-0 bg-cover bg-no-repeat bg-center z-1 mb-[20px] hover:scale-[1.2] transition duration-700" style="background-image: url('/product_photo/@if(@count($product->imagens) > 0){{$product->images->last()->path}}') @else/default/default.jpg @endif">
        </div>
    </a>
